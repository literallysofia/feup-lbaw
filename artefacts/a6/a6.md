...
| Cardinality     | Medium                                                                                                                                                                                                       |
| Clustering      | Yes                                                                                                                                                                                                          |
| Justification   | Query SELECT18 that gets the products of a certain category is executed several times so it has to be fast; doesn´t need range query support; it’s a good candidate for clustering because its cardinality is medium. |
```sql
