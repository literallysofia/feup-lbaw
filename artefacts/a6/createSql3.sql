...
CREATE TABLE product_purchases (
    ...
    /*CONSTRAINT price CHECK ((price > (0)::double precision)),*/
    CONSTRAINT price CHECK (price > 0),
    /*CONSTRAINT quantity CHECK ((quantity > (0)::double precision)),*/
    CONSTRAINT quantity CHECK (quantity > 0)
);
...
CREATE TABLE purchases (
    ...
    "date" TIMESTAMP WITH TIME zone DEFAULT (getdate()) NOT NULL,
    total double precision NOT NULL,
    "user_id" integer NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    "address_id" integer NOT NULL REFERENCES addresses(id) ON DELETE CASCADE,
    /*status text DEFAULT 'Processing'::text NOT NULL*/,
    status text DEFAULT 'Processing' NOT NULL,
    /*CONSTRAINT status CHECK ((TYPE = ANY (ARRAY['Processing', 'Shipped', 'Delivered']))),*/
